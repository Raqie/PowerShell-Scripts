...
1 {Get-ADuser -Filter {EmailAddress -eq $mail} -Properties sAMAccountName, mobile | Select-Object sAMAccountName, mobile  | export-csv .\test.txt -NoTypeInformation -Delimiter ";" -Force -Encoding ASCII  
...
2 {Get-ADuser -Filter {EmailAddress -eq $mail} -Properties sAMAccountName, extensionattribute10 | Select-Object sAMAccountName, extensionattribute10  | export-csv .\testPri.txt -NoTypeInformation -Delimiter ";" -Force -Encoding ASCII  
